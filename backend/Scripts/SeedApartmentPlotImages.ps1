# SeedApartmentPlotImages.ps1
# Reads apartment and open-plot project images from the legacy PerfectJagah assets
# folder and inserts them as binary data into PropertyImages in PerfectJagahDb.
#
# Prerequisites: Run SeedApartmentPlotProperties.sql first.
# Idempotent: properties that already have images are skipped.
#
# Usage:
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\SeedApartmentPlotImages.ps1
#   powershell -NoProfile -ExecutionPolicy Bypass -File .\SeedApartmentPlotImages.ps1 -LegacyImagesPath "D:\other\assets\images"
...
foreach ($project in $projects) {
    ...
    if ($null -eq $propertyId) {
        Write-Warning "Property '$($project.Title)' not found. Run SeedApartmentPlotProperties.sql first."
        continue
    }
